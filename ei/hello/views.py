...
import pandas as pd
from .senti import judge_polarity

# Create your views here.

@login_required(login_url='/admin/login/')
def index(request):
    SF_fan = Anime.objects.filter(genre="SF/ファンタジー") 
    anime_1 = Anime.objects.filter(genre="ロボット・メカ") 
    anime_2 = Anime.objects.filter(genre="アクション・バトル")
    anime_3 = Anime.objects.filter(genre="コメディ・ギャグ")
    anime_4 = Anime.objects.filter(genre="恋愛・ラブコメ")
    anime_5 = Anime.objects.filter(genre="日常・ほのぼの")
    anime_6 = Anime.objects.filter(genre="スポーツ・熱血")
    anime_7 = Anime.objects.filter(genre="ホラー・サスペンス")
    anime_8 = Anime.objects.filter(genre="推理・探偵")
    anime_9 = Anime.objects.filter(genre="歴史・探偵")
    anime_10 = Anime.objects.filter(genre="戦争")
    anime_11 = Anime.objects.filter(genre="ドラマ")
    anime_12 = Anime.objects.filter(genre="青春・学園もの")
    anime_13 = Anime.objects.filter(genre="子供向け")
    anime_14 = Anime.objects.filter(genre="家族向け")
    anime_15 = Anime.objects.filter(genre="短編アニメ")
    anime_16 = Anime.objects.filter(genre="異世界・転生")               
...
def message(request,num):
    ...
    datasss = Message.objects.all().filter(Anime=post.id)

    content_snti = [message_1.content for message_1 in datasss]

    #so = []
    #for i in datasss:
        #so.append(i)
    #content_pojitive = Message.objects.all().filter(Anime=post.id,content=lo) = []
    #listt = []
    #listtt = []
    content_pojitive = []
    content_negative = []
    content_all = []
    for lo in content_snti:
        t = Tokenizer()
    #形態素解析
        tokens = list(t.tokenize(lo)) # ジェネレーターからリストに変換
        pol_val = 0
        ...
        if any(neg in lo for neg in not_list):
            pol_val = pol_val

        if pol_val > 0.3:#極性値の程度が0.3以上だった場合
           content_pojitive.extend(Message.objects.all().filter(Anime=post.id,content=lo))
        elif pol_val < -0.3:
            content_negative.extend(Message.objects.all().filter(Anime=post.id,content=lo))
        else:
            content_all.extend(Message.objects.all().filter(Anime=post.id,content=lo))
    ...
    pa = {
        ...
        'datas' : datas, 
        'contentform' : contentform,
        'content_pojitive':content_pojitive ,
